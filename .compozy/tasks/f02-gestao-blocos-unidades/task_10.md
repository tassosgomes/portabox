...
## Subtasks
- [ ] 10.1 Criar `TestAuthContext` helper para emitir JWT fake aceito pelo `WebApplicationFactory` (se não existir de F01)
- [ ] 10.2 Criar `EstruturaEndpointsTests` cobrindo os 9 endpoints (feliz + erro)
- [ ] 10.3 Criar `SoftDeleteFilterTests` validando query filter e bypass controlado
- [ ] 10.4 Criar `CrossTenantIsolationTests` com 2 tenants e confirmar que tenant A não vê tenant B
- [ ] 10.5 Criar `AuditIntegrationTests` validando audit entries por operação
- [ ] 10.6 Adicionar teste informal de performance (300 unidades, 500ms)
